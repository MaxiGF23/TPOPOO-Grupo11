...
public abstract class AlaFija extends Aeronave{
    private double elasticidadAla; // En porcentaje
    private int cantidadMotores; // Número de motores
    ...
    public void realizarPrueba(){
        ...
        elasticidadAla = 5 + (5 * random.nextDouble()); // 5 a 10%
        ...
        double potenciaTotal = cantidadMotores * (potenciaMinima * (0.8 + (0.8 * random.nextDouble())));
        ...
        if (elasticidadAla >= 5 && elasticidadAla <= 10 && potenciaTotal >= cantidadMotores * potenciaMinima * 0.8) {
